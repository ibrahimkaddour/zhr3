...
class YearYear(models.Model):
    ...
    def check_date_from_to(self):
        ...
        nworking = self.search_count(
            [('date_start', '<=', self.date_stop), ('date_stop', '>=', self.date_start), ('id', '!=', self.id)])
        if nworking:
    ...
    def create_period(self):
        ...
        interval = 1
        for fy in self:
            ds = datetime.strptime(str(fy.date_start), DEFAULT_SERVER_DATE_FORMAT)
